...
- (void)_updateIndicatorAtIndexProgress:(CGFloat)indexProgress animated:(BOOL)animated {
    ...
    CGFloat width = dynamic ? frame.size.width : _indicatorWidth;
...
- (MDHorizontalListViewCell *)_loadCellAtIndex:(NSUInteger)index {
    ...
    _visibleCells[frameString] = cell;
...
- (void)_reloadAtIndex:(NSUInteger)index animated:(BOOL)animated{
    ...
    if (offset != 0) {
        for (NSUInteger i = index + 1; i < _numberOfCells; i++) {
            NSString *originFrameString = _cellFrames[i];
            
            CGRect frame = CGRectFromString(originFrameString);
            frame.origin.x += offset;
            NSString *frameString = NSStringFromCGRect(frame);
            
            MDHorizontalListViewCell *cell = _visibleCells[originFrameString];
            if (cell) {
                [_visibleCells removeObjectForKey:originFrameString];
                _visibleCells[frameString] = cell;
            }
...
- (void)_transitFromCell:(MDHorizontalListViewCell *)fromCell toCell:(MDHorizontalListViewCell *)toCell animted:(BOOL)animated {
    ...
    if (!animated) {
        ...
        toCell.alpha = 0.f;
        NSDictionary *visibleCells = _visibleCells.copy;
        [UIView animateWithDuration:MDHorizontalListViewAnimatedDuration
                         animations:^{
                             fromCell.alpha = 0.f;
                             toCell.alpha = 1.f;
                             [visibleCells enumerateKeysAndObjectsUsingBlock:^(NSString *frameString, MDHorizontalListViewCell *cell, BOOL * stop) {
